...
function _clean(
  ...
  if (!options.ignoreSpecialChar) {
    formatedInput = formatedInput.replace(/[^A-Z0-9]/gi, "");
  }
...
function searchTextInString(
  ...
  const formatedStr = _clean(str, options);
  const results = text
    .split(" ")
    .map((txt) => formatedStr.includes(_clean(txt, options)));
  return options.strict ? results.every(Boolean) : results.some(Boolean);
}
...
function match(enableFuzzyMatch, needle, haystack) {
  ...
  return searchTextInString(needle, haystack, { fuzzy: enableFuzzyMatch });
}
